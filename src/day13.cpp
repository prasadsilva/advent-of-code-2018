    ...
    void read_day13_data(environment_t<env_dim> &outdata, const char* filepath) {
        ...
        for (int y = 0; y < env_dim; y++) {
            for (int x = 0; x < env_dim; x++) {
                ...
                switch (pc) {
                    case '+':   outdata.tracks[y][x].type = track_type_e::intersection; break;
                    case '|':   outdata.tracks[y][x].type = track_type_e::up_down; break;
                    case '-':   outdata.tracks[y][x].type = track_type_e::left_right; break;
                    case '/': {
