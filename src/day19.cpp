...
namespace day19 {
    ...
    struct device_state_t {
        ...
        friend std::ostream & operator << (std::ostream &out, const device_state_t &ds) {
            out << "[" << ds.registers[0] << ", " << ds.registers[1] << ", " << ds.registers[2] << ", " << ds.registers[3] << ", " << ds.registers[4] << ", " << ds.registers[5] << "]";
            return out;
        }
    };
    ...
    struct device_t {
        ...
        void run(const program_t &program, int limit = std::numeric_limits<int>::max(), bool reset_ip = true) {
            ...
            while (instruction_pointer < program.instructions.size() && count < limit) {
                ...
                if (trace1) {
                    std::cout << std::setw(16) << count << " ip: " << std::setw(4) << instruction_pointer << " | " << std::setw(20) << state << "\t -> ";
                }
